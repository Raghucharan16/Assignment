...
def load_models():
    ...
    try:
        ...
        if os.path.exists('models/collaborative_model.pkl'):
            collab_model.load_model('models/collaborative_model.pkl')
        else:
...
def main():
    ...
    if rec_type == "Content-Based Filtering":
        ...
        product_options = {f"{p['title']} ({p['category']})": p['product_id'] 
                          for p in content_model.get_all_products()}
        ...
        selected_user = st.selectbox("Select a user:", all_users)
        
        col1, col2 = st.columns([1, 1])
        
        with col1:
            num_recommendations = st.slider("Number of recommendations", 1, 10, 5, key="collab_slider")
        ...
        if st.button("Get User Recommendations", type="primary"):
            with st.spinner("Analyzing user preferences..."):
                ...
                with col1:
                    st.metric("Total Ratings", user_profile['total_ratings'])
                
                with col2:
                    st.metric("Average Rating", f"{user_profile['average_rating']:.2f}")
        ...
        num_products = st.slider("Number of products to show", 5, 20, 10, key="category_slider")
    ...
    # Footer
    st.markdown("---")
    st.markdown("**Built with ❤️ for Hirebie AI ML Internship**")
    st.markdown("*Powered by Streamlit, scikit-learn, and Surprise*")
